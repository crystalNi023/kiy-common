...
public final class Servo extends Unit {
	...
	protected ConcurrentHashMap<String, Push> pushs;
	//下线通知
	protected ConcurrentHashMap<String, String> offlineNotice;
	/**
	 * 关联天气对象
	 */
	...
	private ConcurrentSet<Device> deviceSet = new ConcurrentSet<>();
	...
	public Servo() {
		...
		pushs = new ConcurrentHashMap<String, Push>();
		offlineNotice = new ConcurrentHashMap<String, String>();
	}
	...
	public void add(Unit u) {
		...
		}else if(u instanceof Push){
			pushs.put(u.id, (Push)u);
		}else {
	...
	public int getUnitCount() {
		return devices.size() + zones.size() + tasks.size() + users.size() + roles.size() + scenes.size() + linkages.size()+pushs.size();
	}
	...
	public Set<Push> getPushs() {
		return new HashSet<>(pushs.values());
	}
	...
	public Set<Device> getSafeProtectionDevices() {
		TreeSet<Device> result = new TreeSet<>(new DeviceComparator());
	 ...
	 * @return 除安防以外所有设备
	 */
	public Set<Device> getExcludeSafeProtectionDevices() {
		TreeSet<Device> result = new TreeSet<>(new DeviceComparator());
		for (Device device : getDevices()) {
			if (Tool.isNotSafeProtectionDevice(device)) {
				result.add(device);
			}
		}
		return result;
	 ...
	 * @return
	 */
	public ConcurrentSet<Device> getDeviceByNumber(String number) {
		deviceSet.clear();
		for (Device device : getDevices()) {
			// 不区分大小写
			if (device.getNumber().toLowerCase().equals(number.toLowerCase())) {
	...
	public void addOfflineNotice(String deviceId,String content){
		offlineNotice.put(deviceId, content);
	}
	/**
	 * 获取下线通知内容
	 * @param deviceId 设备编号
	...
	public String getOfflineNotice(String deviceId){
		return offlineNotice.get(deviceId);
	}
	/**
	 * 移除下线通知内容
	 * @param deviceId
