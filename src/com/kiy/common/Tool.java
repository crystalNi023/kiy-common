...
public final class Tool {
	...
	public static List<Kind> getKindsByVendor(Vendor vender) {
		...
		switch (vender) {
			...
			case EASTSOFT:
				...
				kinds.add(Kind.SENSOR_PM);
				kinds.add(Kind.SENSOR_ILLUMINANCE);
				kinds.add(Kind.SINGLE_FIRE_LINE);
				kinds.add(Kind.HUMAN_INFRARED);
				kinds.add(Kind.NETWORKED_THERMOSTAT);
				kinds.add(Kind.WINDOW_PUSHER);
				kinds.add(Kind.INFRARED_TRANSPONDER);
				kinds.add(Kind.SERIAL_RADIO_FREQUENCY);
				break;
			case DAHUA:
				kinds.add(Kind.ADAPTER);
			...
			case MUDBUS:
				switch (kind) {
					...
					case SERIAL_RADIO_FREQUENCY:
						models.add(Model.MB_0021);
						models.add(Model.MB_0022);
						break;
					default:
						break;
				}
			...
			case EASTSOFT:
				switch (kind) {
					...
					case SWITCH_FOUR://两种
						...
						models.add(Model.ES_0020);
						models.add(Model.ES_0020_01);
						models.add(Model.ES_0020_02);
						models.add(Model.ES_0020_03);
						models.add(Model.ES_0020_04);
						break;
					case SWITCH:
					...
					case SWITCH_TWO:
						...
						models.add(Model.ES_0005_01);
						models.add(Model.ES_0005_02);
						models.add(Model.ES_0005_03);
						break;
					...
					case SENSOR_ILLUMINANCE:
						...
						break;
					case SINGLE_FIRE_LINE:
						models.add(Model.ES_0022);
						models.add(Model.ES_0022_01);
						models.add(Model.ES_0022_02);
						models.add(Model.ES_0022_03);
						break;
					case HUMAN_INFRARED:
						models.add(Model.ES_0023);
						break;
					case NETWORKED_THERMOSTAT:
						models.add(Model.ES_0024);
						break;
					case WINDOW_PUSHER:
						models.add(Model.ES_0025);
						break;
					case INFRARED_TRANSPONDER:
						models.add(Model.ES_0026);
						break;
					case SERIAL_RADIO_FREQUENCY:
						models.add(Model.ES_0027);
						break;
					default:
						break;
				}
	...
	public static List<Vendor> getVendors() {
		...
		vendors.add(8, Vendor.VOLEX);
		vendors.add(9, Vendor.TCPIP);
		vendors.add(10, Vendor.OTHER);
		return vendors;
	 ...
	 * @return
	 */
	public static boolean isNotSafeProtectionDevice(Device device) {
		// 现已使用的非安防的设备只有东软厂商的plc设备，所以此处直接判断设备厂商是否为东软，后续根据需求再做调整；
		// 返回设备不包含中继器（网关）
		if (device.getVendor().equals(Vendor.EASTSOFT) && device.getModel()!=Model.ES_0002)
			return true;
		return false;
	}
	 ...
	 * @return
	 */
	public static boolean isSafeProtectionFYDevice(int m, int u, String deviceName) {
		Model model = Model.valueOf(m);
		switch (model) {
			case MB_0021:
					return true;
	...
	public static boolean isIrrigationDevice(Device device) {
		...
		// 四路开关
			...
			case ES_0006_01:
			case ES_0006_02:
				if (device.getName().contains("灌溉")||device.getName().contains("充氧泵")||device.getName().contains("加热棒")||device.getName().contains("鱼缸")) {
					return true;
				}else {
					return false;
				}
	...
	public static boolean isEnvironmentDevice(Device device) {
		...
		// 温湿度传感器（东软）
			...
			case ES_0006_02:
			case ES_0025:
				if (device.getName().contains("环境")||device.getName().contains("换气")||device.getName().contains("饮水机")||device.getName().contains("水阀")||device.getName().contains("推窗器")) {
					return true;
				}else {
					return false;
				}
	...
	public static boolean isAudioDevice(Device device) {
		...
		// EastSoft 红外转发器(电视)
			...
			case ES_0006_02:
			case ES_0026:
				if (device.getName().contains("影音")||device.getName().contains("遥控器")) {
					return true;
				}else {
					return false;
				}
	 ...
	 * @return
	 */
	public static Device setCombinedEquipment(Device device) {
		
		switch (device.getModel()) {
		case ES_0012:
			//调色灯(默认为开,光度为100)
			device.getFeature(0).setValue(1);
			device.getFeature(1).setValue(100);
			break;
		case ES_0013:
		case ES_0013_01:
			//窗帘(默认为状态控制)
